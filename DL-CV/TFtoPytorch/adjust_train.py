...
if __name__ == '__main__':
    for f, name_xml_train, name_jpg_train in zip(glob(path.join(args.source, "*.xml")), glob("*.xml"), glob("*.jpg")):
        adjust_file(f)
        create_text(name_jpg_train)
        move_file(name_xml_train, args.source, args.dist_xml)
        move_file(name_jpg_train, args.source, args.dist_jpg)
    print("Done!")
